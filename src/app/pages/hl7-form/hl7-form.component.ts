...
export class Hl7FormComponent {
  ...
  startRecording() {
    ...
    this.paciente.enfermedadActual = '';
  ...
  prompt: string = "De la siguiente conversacion extrae informacion del paciente, como nombres, correo, edad solo numeros, cedula, alérgias, síntomas, medicación, con los datos de alergias, sintomas, medicacion dame un prediagnóstico y ponlo en el campo diagnostico, etc. Todo esta info devuelve en un archivo Json para rellenar esta clase paciente = { nombre: '',cedula: '',edad: 0,genero: 'Masculino',direccion: '',celular: '',correo: '',diagnostico: '',alergias: '',sintomas: '',medicacion:'', motivoConsulta: '', enfermedadActual: ''}, en caso de que no hay datos de donde obtener manda un mensaje de que faltan datos: "
    ...
    this.apichatgptService.sendMessage(this.mensaje).subscribe(
      data => {
        ...
        if (responseData.error && responseData.error === 'El texto proporcionado no contiene la información necesaria para extraer los datos del paciente') {
          ...
          this.paciente.motivoConsulta = responseData.motivoConsulta || '';
          this.paciente.enfermedadActual = responseData.enfermedadActual || '';
          this.isLoading = false;
        }
      },
      error => {
